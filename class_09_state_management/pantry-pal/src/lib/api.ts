...
function buildSearchQueryParams(params: RecipesQueryParams): string {
	...
	if (params.maxPrepMinutes) {
		query.set('maxPrepMinutes', String(params.maxPrepMinutes));
	}
...
export async function fetchRecipes(
	...
	try {
		...
		const response = await fetch(
			`${BASE_URL}/recipes${buildSearchQueryParams(params)}`,
		);
